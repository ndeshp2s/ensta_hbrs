...
namespace simple_layer_namespace
{
	...
	void SimpleLayer::updateBounds(double origin_x, double origin_y, double origin_yaw, double* min_x, double* min_y, double* max_x, double* max_y)
	{
		if (!enabled_)
			return;
		...
		*max_x = std::min(*max_x, mark_x_);
		*max_y = std::min(*max_y, mark_y_);
	}
